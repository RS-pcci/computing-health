...
Dallas_County = 2622799
S_default = Dallas_County
known_infections = 303 # update daily
known_cases = 50 # update daily
...
relative_contact_rate = st.sidebar.number_input(
    "Social distancing (% reduction in social contact)", 0, 100, value=5, step=5, format="%i"
)/100.0
...
Parkland_market_share = (
    st.sidebar.number_input(
        "Hospital Market Share (%)", 0.0, 100.0, value=20.0, step=1.0, format="%f"
    )
    / 100.0
)
...
initial_infections = st.sidebar.number_input(
    "Currently Known Regional Infections (only used to compute detection rate - does not change projections)", value=known_infections, step=1, format="%i"
)

total_infections = current_hosp / Parkland_market_share / hosp_rate
...
def head():
    ...
    st.markdown(
        """*This tool was developed by [Parkland Center for Clinical Innovation](https://pccinnovation.org/) based on the application developed by the Predictive Healthcare team at Penn Medicine. For questions and comments please see our
    [contact page](https://pccinnovation.org/contact/). Code can be found on [Github](https://github.com/RS-pcci/computing-health).
    )
    ...
    and Hospital market share (**{Parkland_market_share:.0%}**).
...
""".format(
        ...
        hosp_rate=hosp_rate,
        S=S,
        Parkland_market_share=Parkland_market_share,
...
The epidemic proceeds via a growth and decline process. This is the core model of infectious disease spread and has been in use in epidemiology for many years."""
    ...
    st.markdown(
        """To project the expected impact to Parkland Medicine, we estimate the terms of the model.
...
- The total size of the susceptible population will be the entire catchment area for Parkland Hospital entities
  - Dallas County = {Dallas_County}""".format(
            Dallas_County=Dallas_County,
        )
    )
    return None
...
hosp = i * hosp_rate * Parkland_market_share
icu = i * icu_rate * Parkland_market_share
vent = i * vent_rate * Parkland_market_share

# Recovered
r_hosp = r * hosp_rate * Parkland_market_share
r_icu = r * icu_rate * Parkland_market_share
r_vent = r * vent_rate * Parkland_market_share
...
st.markdown("Projected number of **daily** COVID-19 admissions at Parkland Hospital")
...
st.markdown(
    "Projected **census** of COVID-19 patients, accounting for arrivals and discharges at Parkland Hospital"
)
